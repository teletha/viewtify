...
public class NTooltip extends PopupControl {
    ...
    private final StyleableObjectProperty<Duration> openDurationProperty = new SimpleStyleableObjectProperty<>(OPEN_TIME_CMD, this, "openDuration");
    ...
    private final StyleableObjectProperty<Duration> hideDurationProperty = new SimpleStyleableObjectProperty<>(HIDE_TIME_CMD, this, "hideDuration");
    ...
    private final StyleableObjectProperty<Duration> leftDurationProperty = new SimpleStyleableObjectProperty<>(LEFT_TIME_CMD, this, "leftDuration");
    ...
    private final StyleableBooleanProperty popupNonFocusProperty = new SimpleStyleableBooleanProperty(POPUP_NONFOCUS_CMD, this, "popupNonFocus", false);
    ...
    private final StyleableBooleanProperty hideOnExitProperty = new SimpleStyleableBooleanProperty(HIDE_ON_EXIT_CMD, this, "hideOnExit", false);
    ...
    private final StyleableDoubleProperty xOffsetProperty = new SimpleStyleableDoubleProperty(XOFFSET_CMD, this, "xOffset", TOOLTIP_DEFAULT_XOFFSET);
    ...
    private final StyleableDoubleProperty yOffsetProperty = new SimpleStyleableDoubleProperty(YOFFSET_CMD, this, "yOffset", TOOLTIP_DEFAULT_YOFFSET);

    // ---------------------------------------------------------
    // CSS Meta Data ------
    // ------------------------------------------------------------------------
    private static final CssMetaData<CSSBridge, Duration> OPEN_TIME_CMD = new CssMetaData<CSSBridge, Duration>("wait-time", getDurationConverter()) {
    ...
    private static final CssMetaData<CSSBridge, Duration> HIDE_TIME_CMD = new CssMetaData<CSSBridge, Duration>("visible-time", getDurationConverter()) {
    ...
    private static final CssMetaData<CSSBridge, Duration> LEFT_TIME_CMD = new CssMetaData<CSSBridge, Duration>("closing-time", getDurationConverter()) {
    ...
    private static final CssMetaData<CSSBridge, Boolean> POPUP_NONFOCUS_CMD = new CssMetaData<CSSBridge, Boolean>("popup-nonfocus", getBooleanConverter(), false) {
        ...
        @Override
        public StyleableProperty<Boolean> getStyleableProperty(final CSSBridge styleable) {
            return styleable.tooltip.popupNonFocusProperty;
        }

    };

    private static final CssMetaData<CSSBridge, Boolean> HIDE_ON_EXIT_CMD = new CssMetaData<CSSBridge, Boolean>("hide-on-exit", getBooleanConverter(), false) {
    ...
    private static final CssMetaData<CSSBridge, Number> XOFFSET_CMD = new CssMetaData<CSSBridge, Number>("offset-x", getSizeConverter(), TOOLTIP_DEFAULT_XOFFSET) {
    ...
    private static final CssMetaData<CSSBridge, Number> YOFFSET_CMD = new CssMetaData<CSSBridge, Number>("offset-y", getSizeConverter(), TOOLTIP_DEFAULT_XOFFSET) {
    ...
    /**
     * The text to display in the tooltip. If the text is set to null, an empty string will be
     * displayed, despite the value being null.
     */
    ...
    /**
     * Specifies the behavior for lines of text <em>when text is multiline</em>. Unlike
     * {@link #contentDisplayProperty() contentDisplay} which affects the graphic and text, this
     * setting only affects multiple lines of text relative to the text bounds.
     */
    ...
    private final ObjectProperty<TextAlignment> textAlignment = new SimpleStyleableObjectProperty<>(TEXT_ALIGNMENT, this, "textAlignment", TextAlignment.LEFT);

    /**
     * Specifies the behavior to use if the text of the {@code Tooltip} exceeds the available space
    ...
    /**
     * If a run of text exceeds the width of the Tooltip, then this variable indicates whether the
     * text should wrap onto another line.
     */
    ...
    /**
     * The default font to use for text in the Tooltip. If the Tooltip's text is rich text then this
     * font may or may not be used depending on the font information embedded in the rich text, but
     * in any case where a default font is required, this font will be used.
     */
    ...
    private final ObjectProperty<Font> font = new StyleableObjectProperty<Font>(Font.getDefault()) {
        ...
        @Override
        public void applyStyle(final StyleOrigin newOrigin, final Font value) {
            // RT-20727 - if CSS is setting the font, then make sure invalidate doesn't call
            // impl_reapplyCSS
            try {
        ...
        @Override
        public void set(final Font value) {
    ...
    /**
     * An optional icon for the Tooltip. This can be positioned relative to the text by using the
     * {@link #contentDisplayProperty() content display} property. The node specified for this
     * variable cannot appear elsewhere in the scene graph, otherwise the
     * {@code IllegalArgumentException} is thrown. See the class description of
     * {@link javafx.scene.Node Node} for more detail.
     */
    ...
    private StyleableStringProperty imageUrlProperty() {
        if (imageUrl == null) {
            imageUrl = new StyleableStringProperty() {
                ...
                @Override
                public void applyStyle(final StyleOrigin origin, final String v) {
    ...
    private final ObjectProperty<ContentDisplay> contentDisplay = new SimpleStyleableObjectProperty<>(CONTENT_DISPLAY, this, "contentDisplay", ContentDisplay.LEFT);
    ...
    private final DoubleProperty graphicTextGap = new SimpleStyleableDoubleProperty(GRAPHIC_TEXT_GAP, this, "graphicTextGap", 4d);

    /**
     * Typically, the tooltip is "activated" when the mouse moves over a Control. There is usually
     * some delay between when the Tooltip becomes "activated" and when it is actually shown. The
     * details (such as the amount of delay, etc) is left to the Skin implementation.
     */
    private final ReadOnlyBooleanWrapper activated = new ReadOnlyBooleanWrapper(this, "activated");

    final void setActivated(final boolean value) {
        activated.set(value);
    }
    ...
    private static final CssMetaData<CSSBridge, Boolean> WRAP_TEXT = new CssMetaData<CSSBridge, Boolean>("-fx-wrap-text", BooleanConverter
    ...
    private static final CssMetaData<CSSBridge, String> GRAPHIC = new CssMetaData<CSSBridge, String>("-fx-graphic", StringConverter
            .getInstance()) {
    ...
    private static final CssMetaData<CSSBridge, Number> GRAPHIC_TEXT_GAP = new CssMetaData<CSSBridge, Number>("-fx-graphic-text-gap", SizeConverter
            .getInstance(), 4.0) {
    ...
    static {
        final ArrayList<CssMetaData<? extends Styleable, ?>> list = new ArrayList<>(PopupControl.getClassCssMetaData());
