...
public abstract class TooltipBehaviorBase<P extends PopupWindow> extends SinglePopupBehavior<P> {
    ...
    protected void show(final P p, final Node hover, double x, double y) {
        ...
        p.show(owner, Math.floor(x + ox), Math.floor(y + oy));

        if ((y + TOOLTIP_YOFFSET) > p.getAnchorY()) {
            p.hide();
            y -= p.getHeight();
            p.show(owner, Math.floor(x + ox), Math.floor(y));
        }
    }
    ...
    protected static final Duration DEFAULT_OPEN_DURATION = new Duration(1000), DEFAULT_HIDE_DURATION = new Duration(5000),
